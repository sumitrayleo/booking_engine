...
import com.cognizant.orchestration.dto.OrchestrationError;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler
    public ResponseEntity<ErrorResponse> handle(InvalidApiKeyException exception, HttpServletRequest httpServletRequest) {

        HttpHeaders headers = new HttpHeaders();

        LOGGER.error("InvalidApiKeyException - " + appendClientInformation(httpServletRequest));

        headers.setContentType(MediaType.APPLICATION_JSON);

        ErrorResponse response = new ErrorResponse();
        response.getErrors().add(new OrchestrationError(ReturnCodeEnum.INVALID_API_KEY.toString(), exception.getMessage()));

        return new ResponseEntity<ErrorResponse>(response, headers, HttpStatus.FORBIDDEN);
    }
    ...
    @ExceptionHandler
    public ResponseEntity<ErrorResponse> handle(final BookingApplException exception, HttpServletRequest httpServletRequest) {
        HttpHeaders headers = new HttpHeaders();
