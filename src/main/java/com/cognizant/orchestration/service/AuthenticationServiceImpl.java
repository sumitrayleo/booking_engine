...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public LoginRS login(final LoginRequest loginRequest) {
        ...
        final String msg = String.format("Processing cleartext login request, username %s", loginRequest.getUsername());
        LOGGER.debug(msg);

        try {
            return populateLoginResponse();
        } catch (Exception e) {
            throw new BookingApplException("Authenticiation Exception", e);
        }
